  ...
  byId("helpContent").innerHTML = `
    ...
    <div id="helpBody" class="help-body" tabindex="0">
      ...
      <section id="help-flow" class="help-section">
        ...
        <h5>操作手順</h5><ol><li>収入管理で対象月の給与を保存します。</li><li>外部データでMoneyForwardまたは楽天カードCSVを取り込みます。</li><li>支出管理の登録データ画面で更新候補バッジと差額を確認します。</li><li>必要な候補だけ確認モーダルから反映します。</li><li>サマリーと分析で支出設計の健全性を確認します。</li></ol>
      ...
      <section id="help-expense" class="help-section">
        ...
        <h5>できること</h5><ul><li>登録データタブでカード表示を中心に支出項目を確認できます。</li><li>支払者を「すべて」または個別に切り替えられます。</li><li>支出種別、固定/変動、必須/任意、削減可能かを設定できます。</li><li>毎月、毎年、隔月、半年の支払い時期に対応します。</li></ul>
        <h5>操作手順</h5><ol><li>支出管理の登録データタブを開きます。</li><li>支払者を選びます。全体を見る場合は「すべて」を選びます。</li><li>カードまたはリストから項目を選びます。</li><li>詳細カードで編集を押し、必要項目を変更します。</li><li>保存します。金額変更は履歴として残ります。</li></ol>
      ...
      <section id="help-candidates" class="help-section">
        ...
        <h5>操作手順</h5><ol><li>支出管理の登録データ画面で更新候補がある項目を開きます。</li><li>未確認、差額あり、信頼度、データ元で絞り込みます。</li><li>根拠を確認します。</li><li>変更する場合は反映するを押します。</li><li>確認モーダルで変更前後を見て、反映します。</li></ol>
      ...
      <section id="help-analysis" class="help-section">
        ...
        <h5>操作手順</h5><ol><li>左メニューの分析を開きます。</li><li>最上段の要点を確認します。</li><li>見直し候補Top5を確認します。</li><li>必要に応じて支出管理の登録データまたは外部データへ移動します。</li></ol>
      ...
      <section id="help-data" class="help-section">
        ...
        <h5>できること</h5><ul><li>収入管理では登録月データの確認、Excel出力、直近データ復元ができます。</li><li>支出管理では登録データ画面下部からCSVエクスポートできます。</li><li>バックアップと復元は設定内のバックアップに集約しています。</li></ul>
        <h5>操作手順</h5><ol><li>収入管理の登録データ、または支出管理の登録データ下部を開きます。</li><li>一覧を確認します。新しい月が上に表示されます。</li><li>必要に応じて出力または履歴確認を行います。</li></ol>
...
function uiExternalSourceType(rowOrValue) {
...
function externalLinkOptions() {
  ...
  importedRows
    .filter((row) => uiExternalSourceType(row) === uiExternalSourceType(source))
    .filter((row) => {
    ...
    .forEach((row) => {
      ...
      if (!name) return;
      const normalizedSource = uiExternalSourceType(source);
      const key = `${normalizedSource}:${normalize(name)}`;
      const current = groups.get(key) || { key, source: normalizedSource, name, rows: [], latestMonth: "", total: 0 };
      current.rows.push(row);
      const ym = uiExternalMonth(row);
  ...
  return [...groups.values()]
    .map((group) => ({
      ...group,
      total: group.rows
        .filter((row) => !group.latestMonth || uiExternalMonth(row) === group.latestMonth)
